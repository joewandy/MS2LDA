...
\TomotopyEvaluable{} and \TomotopyUseful{}. Thus Contextual Sparse ETM produced
\ContextualEvaluableGainVsTomotopy{} more evaluable and
\ContextualUsefulGainVsTomotopy{} more useful motifs. This breadth gain was
accompanied by lower conditional mean SOS (\ContextualMeanSOS{} versus
\TomotopyMeanSOS{}) and a lower useful fraction of the evaluable inventory
(\ContextualUsefulRate{} versus \TomotopyUsefulRate{}). Held-out half-spectrum
completion negative log-likelihood (NLL) was \ContextualNLL{} versus
\TomotopyNLL{}. Across three training seeds on the same split, evaluable and
useful motif counts remained within
...
topics became the largest component for at least one test spectrum. These results
support a reproducible discovery-breadth gain with a quantified conditional-quality
trade-off relative to LDA, while preserving sparse spectrum assignments on the
held-out test split.
\end{abstract}
...
creating a large unfiltered topic inventory. Before collapsing repeated
connectivity keys within topics, every method contributes exactly
\TestSpectra{} dominant-topic spectrum associations. The comparison therefore
does not give one model more opportunities through an absolute posterior cutoff.
...
spectrum--topic associations before connectivity-key deduplication.}
\label{fig:inventory}
\end{figure}
...
Tomotopy remains higher on conditional mean SOS. The dense ETM controls also
have higher conditional mean SOS but expose much smaller evaluable inventories.
...
Relative to Tomotopy, the proposed model yields
\ContextualEvaluableGainVsTomotopy{} more evaluable motifs and
\ContextualUsefulGainVsTomotopy{} more useful motifs, while lowering completion
NLL from \TomotopyNLL{} to \ContextualNLL{}.

The result is not uniform dominance. Useful motifs constitute
\ContextualUsefulRate{} of the proposed model's evaluable inventory and
\TomotopyUsefulRate{} of Tomotopy's, a difference of
\ContextualUsefulRateDifferenceVsTomotopy{} percentage points. Tomotopy's
conditional mean SOS is higher by \ContextualMeanSOSDifferenceVsTomotopy{}
(\TomotopyMeanSOS{} versus \ContextualMeanSOS{}). Canonical and balanced ETM also
have higher conditional mean SOS and lower completion NLL but produce
substantially fewer evaluable and useful motifs. The primary result is therefore
increased discovery breadth with an explicit conditional-quality trade-off
relative to Tomotopy and a predictive-fit trade-off relative to dense ETM.
...
more motifs eligible for SOS and more useful motifs than canonical ETM,
fragment- and loss-balanced ETM and Tomotopy, including
\ContextualEvaluableGainVsTomotopy{} and \ContextualUsefulGainVsTomotopy{} gains,
respectively, over Tomotopy, while retaining
...
conditional mean SOS and useful/evaluable rate, while dense ETM retains better
completion likelihood. The scientific
conclusion is a reproducible discovery-breadth advantage on this held-out split
with quantified conditional-quality and predictive trade-offs. External data
