...
retains the published ETM generator, Gaussian variational encoder and
...
posterior and channel normalization to meet that target without introducing a
...
\subsection{Context is necessary for useful sparse recovery in short spectra}
...
contribute differently when its neighboring evidence changes. Restricting each
...
Contextual Sparse ETM preserves ETM's generator and variational objective while
adding equal fragment/loss decoder mass, contextual top-2 posterior evidence
and $1.5$-entmax. Only one learned scalar is added.
...
\ContextualEvaluableGainVsTomotopy{} and
\ContextualUsefulGainVsTomotopy{} across three initializations, while sparse
