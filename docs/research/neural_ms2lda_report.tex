...
spectra and \ValidationSpectra{} validation spectra. The model produced
...
statistics use only the training partition. The present study uses training and
validation evidence; the test partition was not loaded or scored for the
proposed model.

For predictive evaluation, validation spectra are deterministically divided
50/50 by whole physical peak groups. A fragment and its corresponding neutral
...
Finally, each vocabulary token has a fixed 48-dimensional skip-gram
...
central processing unit (CPU) threads support training on an NVIDIA
CUDA-compatible graphics processing unit (GPU).
...
connectivity InChIKey is removed before constructing the FAISS
vector-similarity search index. The evaluated molecule therefore cannot be
retrieved as its own annotation evidence.
...
entropy-effective topic count, support counts exactly non-zero document-topic
weights, and unique counts distinct document winners.}
\label{tab:high-k}
...
\end{groupplot}
\end{tikzpicture}
\caption{Validation motif inventory after successive chemical filters. ETM is
...
use $K=1000$ and the same frozen training/validation split and
...
\centering
\small
\caption{Validation comparison. SOS is the substructure overlap score and NLL
...
validation split. Horizontal position counts motifs for which the substructure
...
\centering
\footnotesize
\caption{Validation sparsity and inventory diagnostics. Eff. is the
...
\ValidationThroughput{} full validation spectra per second. Peak memory actively
...
memory, including CPU memory, is \PeakProcessGB{} GB, while at least
...
This is a validation study on one positive-ion-mode MS$^{n}$Lib benchmark. The
...
SHA-256 file checksums. Test artifacts for the proposed model are explicitly
excluded from the current evidence boundary.

Every numerical fragment in this report is generated from frozen
