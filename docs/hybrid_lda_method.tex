...
results from that execution were superseded. The peak-aware rerun converged and
completed every frozen evaluation stage. In this post-hoc single-seed result,
two-step inference closely approached long local refinement, and the
compound-balanced SOS diagnostic was comparable between HybridLDA and
Tomotopy. Word co-occurrence and likelihood results were mixed, so the result
does not establish general superiority. MS2LDA is a research project at this
stage; software validation is not presented as chemical evidence or as a
production-system migration decision.
\end{abstract}
...
NPMI uses binary training-document occurrence of each topic's top ten words.
Following the frozen implementation, a top-word pair that never co-occurs has
undefined NPMI and is assigned the limiting worst score of $-1$ rather than
being omitted; the machine-readable model artifacts retain the number of such
pairs.
...
claims come from its completed generated report.}\label{tab:nature-comparison}\\
...
question of benchmark-unseen compounds. A comparability claim was withheld
until the corrected generated report was complete. That gate is now satisfied;
its within-benchmark SOS comparison is reported below, while the difference in
fingerprints and thresholds prevents direct numerical comparison with the
paper's deposited SOS values.
...
one thread for all held-out inference and latency measurements. The corrective
Hybrid fit started from epoch zero and satisfied the unchanged joint
topic-word/alpha convergence rule after 149 discovery epochs, within the frozen
250-epoch safety ceiling. Discovery took 34,600.9 seconds and finalization
827.3 seconds. The asymmetric alpha concentration fell from its initialization
sum of 600 to 9.1117 using training documents only. No older Hybrid feature,
checkpoint, model, chemical mixture, or MAG output was imported.
...
The replacement passed every acceptance gate: peak-aware features were
rebuilt; Hybrid converged from epoch zero; all inference arms completed;
chemical inference used full held-out spectra after labels had been withheld
from fitting; and the MAG index excluded 53,426 of 426,280 reference rows with
zero held-out rows retained. Independent recalculation also found zero
split-group and compound-identity leakage, verified that all four
$7777\times1000$ test-mixture matrices were finite and normalized, and
reproduced the per-topic SOS summaries below.

The main semi-amortized result is positive but qualified. Encoder-only
inference reached mean/median cosine 0.9270/0.9533 against the validated
long-refinement mixture, but its fifth percentile was only 0.7740. Two local
VB steps raised these values to 0.9893/0.9996 with a fifth percentile of
0.9420 and mean Jensen--Shannon divergence 0.0119. Thus two steps remove most
of the tail error that remains in the pure encoder. Cached inference took
1.888 ms per spectrum for the encoder and 2.937 ms for two steps, versus
19.375 ms for long refinement and 58.633 ms for Tomotopy. Including DreaMS
extraction reduces the advantage: encoder and two-step end-to-end times were
32.839 and 33.935 ms, versus 51.004 ms for long refinement and 58.743 ms for
Tomotopy.

Predictive likelihood does not improve monotonically with refinement. The
encoder's held-out NLL was 9.7655 per in-vocabulary token, close to Tomotopy's
9.7569, whereas two-step and long refinement gave 10.0788 and 10.1882.
Local VB optimizes the observed completion half rather than its unobserved
partner, so convergence toward the long reference is not equivalent to better
document-completion prediction. The benchmark therefore supports the claim
that two-step inference approximates the chosen long-inference procedure; it
does not support a claim that refinement improves held-out likelihood.

The primary compound-balanced SOS diagnostic used every held-out spectrum once
through dominant-topic association. Tomotopy had 598 evaluable topics with
mean SOS 0.6232. Hybrid had 608, 619, and 625 evaluable topics for encoder,
two-step, and long inference, with means 0.6300, 0.6307, and 0.6293. These small
differences are descriptive, not evidence of superiority from one seed. The
unchanged probability-$\geq0.5$ sensitivity associated only 961 Tomotopy test
spectra and 472, 97, and 115 Hybrid spectra, producing 333, 181, 55, and 65
evaluable topics respectively. That sensitivity confirms that the absolute
cutoff is not calibrated across inference algorithms; it is not the primary
chemical comparison.

Other diagnostics prevent a one-sided interpretation. Hybrid had slightly
higher top-word diversity (0.8387 versus 0.8148) but worse mean word
co-occurrence NPMI ($-0.4744$ versus $-0.2997$). Of the 45,000 top-word pairs
scored for each method, 25,599 Hybrid pairs and 20,537 Tomotopy pairs never
co-occurred and therefore received the frozen $-1$ score. Thus, the NPMI gap
partly reflects more disconnected Hybrid top-word pairs; it is not merely a
difference among pairs with observed co-occurrence. Optimally matched
Tomotopy--Hybrid topics had mean cosine 0.4768 and mean top-word Jaccard 0.0816,
showing that comparable aggregate SOS does not imply that the methods discovered
the same motifs. MAG annotations were available for 607 Tomotopy and 631
Hybrid topics; among the 475 matched pairs annotated by both methods, mean
MACCS-consensus Jaccard was 0.5483. Raw DreaMS nearest neighbours had mean
Morgan similarity 0.2207 and no exact connectivity or scaffold matches under
the held-out split. These are similarity and coherence diagnostics, not
fragment-assignment correctness.

The final tables below are generated directly from the hash-verified report;
they are not manually transcribed.
...
inference improvement cannot replace external chemical validation. SOS and MAG
provide real but indirect chemical evidence; they do not replace the unavailable
independent manual motif--spectrum endpoint. The completed correction supports
only the stated single-seed, post-hoc comparison, and production-readiness
criteria are outside this research project.
...
explain provenance, not as chemical evidence. The executed corrective protocol
is \hash{3b2a07029b22c51c682cf11e0756bcdd2f6092878e67cd2db34df82caced5b7b}
at source commit
\hash{98601867ec3ace2fb3c51ec50ed2f2305eba608b}. It hashes the benchmark,
Hybrid implementation, and the exact local MS2LDA modules used by MAG. Its
compact checkpoint and LaTeX fragment are exported mechanically
...
features. Its completed post-hoc run shows that two local VB steps approximate
the long-refinement mixtures much more closely than encoder-only inference at
near-encoder latency. Under the compound-balanced dominant-topic SOS
diagnostic, all Hybrid inference arms have coverage and mean coherence
comparable to Tomotopy; however, Hybrid has worse word-co-occurrence NPMI, the
methods' matched topics overlap only modestly, and refinement worsens held-out
document-completion NLL. The result is therefore indicative evidence that the
semi-amortized speed/coherence trade-off is scientifically promising, not a
universal ranking, proof of individual fragment assignments, substitute for
independent manual validation, or production-backend decision.
