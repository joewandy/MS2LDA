...
one thread for all held-out inference and latency measurements. It permits at
most 50 discovery epochs with early stopping and retains the newest two atomic
checkpoints. If Hybrid does not satisfy both global convergence criteria, the
driver retains the latest checkpoint and stops before encoder finalization,
MAG, or reporting.

\subsection{Results and discussion}

Corrected real-data results are pending. This section will be populated only
from the completed, hash-verified report under the active run path above. The
historical SOS membership threshold remains frozen at 0.5; it will not be
lowered after inspecting the corrected mixtures.
