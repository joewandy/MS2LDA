...
inside topic discovery. The implementation exposes a small Tomotopy-shaped
...
before declaring finalization successful. If optimization or a progress
callback fails, it restores the encoder, random-generator state, history, and
module mode. Topic discovery cannot resume after successful finalization, and
new-document inference and checkpoint saving are unavailable beforehand.
...
tolerance makes that number a maximum batch-level budget. Adaptive stopping is
never used in the differentiable training
...
classical discovery, inference finalization, topic access, new-document
inference, document topic distributions, likelihood summaries, and save/load.
It is not wired into the existing CLI, dashboard, or visualizer. Tomotopy
...
A second experiment used the archived random-split mushroom artifacts only to
freeze the same discovered topics and reconstruct the exact split: 2,171
training spectra, 539 eligible test spectra, 10,029 training-vocabulary words,
209,497 observed test tokens, and 214,271 held-out test tokens. For each topic
seed 42--46, both comparison encoders were reset to the same fresh seeded
initialization before any final-stage training. Each received the same frozen
topics, document order, minibatch random-number-generator state, and 12 epochs.
The benchmark comparator regressed to normalized local-VB posterior means; the
current method optimized Equation~\ref{eq:semi-amortized-objective}. Test
embeddings came from partial spectra containing observed physical peaks only.

The semi-amortized arm reduced the two-step signed ELBO gap in all five seeds:
7.39--10.52\%, with mean 9.53\% and sample standard deviation 1.38 percentage
points. Three of five seeds exceeded a prespecified 10\% diagnostic threshold;
the threshold was retained rather than revised after observing the rerun.
Held-out NLL was lower in all five seeds at both zero and two refinement steps.
The mean improvements were 0.0481 nats/token at zero steps and 0.0146 at two
steps; mean two-step NLL was 7.0048 rather than 7.0194. The current method at
two steps was also 0.0499 nats/token better than the comparator after five
steps. Every topic and structured-prior tensor remained bitwise unchanged.

Long local solves were diagnostics, not claims of uniform numerical
convergence. Across seeds, one additional coordinate update was positive for
as many as 107 documents and had a maximum individual gain of 0.9063 nats. The
largest corpus-average continuation gain was only
$1.02\times10^{-5}$ per observed token, too small to explain the finite-budget
differences. As an artifact-integrity check, migrated predictions reproduced
stored NLL values with mean absolute error $1.28\times10^{-8}$ and maximum
absolute error $3.71\times10^{-8}$ nats/token.

The synthetic driver is
\path{scripts/benchmark_semi_amortized_inference.py}; the trusted-artifact
mushroom driver is
\path{scripts/benchmark_mushroom_inference_phase.py}. The regression comparator
lives only in \path{scripts/inference_baselines.py}; it is not exposed by the
production model or serializable as a finalized checkpoint. Both drivers retain
machine-readable raw results. These experiments support a narrow claim: from a
fresh seeded encoder and fixed topics, the final semi-amortized encoder is a
better limited-budget document-inference initializer than this comparator.
They do not show that discovered motifs are more chemically meaningful, more
...
weight were selected in an internal simulator; the fresh-encoder mushroom
rerun is a useful robustness check but not an independent model-selection
study. Finalization intentionally locks topic discovery, so fitting on a
premature topic state cannot be repaired without training a new model. The
model is not an end-to-end neural topic model, and the production MS2LDA
workflow does not yet select it as a backend. Finally, a passing software test
suite and frozen-topic inference improvement cannot replace external chemical
validation.
...
\path{tests/test_hybrid_lda.py}, \path{scripts/inference_baselines.py}, the two
benchmark drivers named above, \path{environment-hybrid.yml}, and this document.
A small lazy import in \path{MS2LDA/__init__.py} prevents the full application
stack from loading when the reference modules are imported.
