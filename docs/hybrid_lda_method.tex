...
results from that execution were superseded. A later audit found that the first
peak-aware correction could still lend one retained DreaMS state to a nearby
peak discarded by DreaMS's top-100 preprocessing. Those Hybrid-dependent
results are also superseded. The implementation now requires exact retained
peak identity and authenticated resumable feature caches; its replacement
full-data run is pending. MS2LDA is a research project at this stage; software
validation is not presented as chemical evidence or as a production-system
migration decision.
\end{abstract}
...
full-data seed-42 benchmark is designed to test fast local inference and
chemically coherent topic discovery on benchmark-unseen compounds. This
document reports the complete design, both result-changing peak-pooling defects
found during review, and the frozen replacement protocol without selecting
favourable arms. The
...
physical peak, the implementation accepts a contextual state only when the
state's float32 m/z identity exactly equals that source peak's identity. The
fragment and neutral-loss words originating from that peak receive the same
state, while repeated occurrences contribute according to their counts. A
source peak discarded by DreaMS's top-100 preprocessing remains unmatched and
cannot borrow a nearby retained peak's state. Duplicate float32 source or
retained identities fail closed. Pooling these exact assignments gives a mean
contextual vector $\bar{\mathbf h}_v$ for each word. Unmatched words receive a
zero contextual vector but retain their mass and word-type features.
...
snapshot for the replacement run. The authoritative machine-readable values
remain the committed seed-42 configuration and the protocol lock created before
execution; no parameter below is selected from chemical labels.
...
change; patience 5; run must stop only after satisfying patience \\
...
long refinement of 50 updates or a validation-triggered extension to 100; one
inference thread \\
...
\end{tabular}
\caption{Complete parameter snapshot for the frozen seed-42 replacement run.
The 250-epoch ceiling is a safety limit; a run that reaches it without satisfying
both global criteria is rejected.}
...
epochs in the original five-seed profile. The seed-42 replacement raises only
that safety ceiling to 250 and, after discovery convergence, runs 12
inference-encoder epochs. All convergence decisions use training or validation
quantities, never chemical labels or test likelihood.
...
methods. ``This benchmark'' refers to the frozen seed-42 replacement design;
Hybrid numerical claims remain pending.}\label{tab:nature-comparison}\\
...
after defects were found in both earlier Hybrid feature pools. The replacement
must complete before any within-benchmark SOS comparison is reported. Even
then, the difference in fingerprints and thresholds prevents direct numerical
comparison with the paper's deposited SOS values.
...
Independent review first found that the training-only word pool iterated over a
document-level token counter. When separate physical peaks rounded to the same
fragment or neutral-loss word, their combined multiplicity was assigned to only
one contextual state. An attempted correction iterated over physical peak
groups, but matched each group independently to the nearest retained DreaMS
peak within 0.02 Da. Because the frozen DreaMS model retains only 100 peaks, a
discarded group could still borrow the state of a nearby retained group.

A read-only audit of all 27,222 training spectra found 158 spectra with this
state aliasing, comprising 708 reused states and 743 extra physical groups. In
53 spectra, 115 extra groups also shared the retained group's rounded word.
These groups change the pooled word vectors used by Hybrid's structured prior,
so every Hybrid-dependent result from that execution is superseded. Tomotopy,
the split, vocabulary, and leakage audits do not consume these states and remain
valid.

The replacement implementation accepts a contextual state only when its
float32 m/z identity exactly equals the source physical peak. A peak discarded
by DreaMS is unmatched rather than assigned a neighbour's state, and ambiguous
duplicate identities fail closed. Both training and full-test embedding caches
authenticate identifiers and every completed row chunk before resuming. The
replacement starts Hybrid from epoch zero and reuses only the byte-verified
Tomotopy core model. It remains explicitly post-hoc because previous seed-42
results were inspected before the correction was designed:
{\footnotesize
\begin{verbatim}
REPO=/path/to/MS2LDA
DATA=/path/to/ms2lda-msnlib-validation/zenodo/20179680
...
one thread for all held-out inference and latency measurements. The replacement
run is pending. No earlier DreaMS feature pool, Hybrid checkpoint, Hybrid model,
chemical mixture, MAG result, or SOS result is eligible for import.

\subsection{Results status}

No Hybrid chemical result is currently reported. The nearest-state correction
did complete, but its top-100 aliasing defect changes the structured prior and
therefore every Hybrid-derived result. Its checkpoint, tables, and numerical
interpretation have been removed rather than retained as an alternative
analysis. The hash-verified Tomotopy fit, frozen scaffold split, training-only
vocabulary, and leakage controls remain eligible inputs to the replacement,
but they cannot validate the affected Hybrid topics retrospectively.

Results will be exported only after exact peak-identity pooling reconciles all
eligible groups as matched or unmatched, all Hybrid stages rebuild from epoch
zero, and the full publication and leakage audits pass. Until then, smoke tests
and synthetic benchmarks are software validation only, not chemical evidence.
...
independent manual motif--spectrum endpoint. The replacement must remain
uninterpreted until every frozen stage completes, and production-readiness
...
explain provenance, not as chemical evidence. Source commit
\hash{98601867ec3ace2fb3c51ec50ed2f2305eba608b} and protocol
\hash{3b2a07029b22c51c682cf11e0756bcdd2f6092878e67cd2db34df82caced5b7b}
identify the superseded nearest-state correction. Its publication checkpoint,
generated tables, and manifest have been removed. The replacement protocol
will record the immutable corrected source, benchmark code, and exact local
MS2LDA modules used by MAG before execution. Large models, DreaMS caches, FAISS
indices, and complete generated reports remain outside Git.

The public inputs can be reconstructed without local history using the
standard-library, resumable downloader. A fresh replacement seed-42 run starts
from the immutable implementation commit shown below and uses an empty output
directory. The final protocol lock independently records the executing
checkout and the hashes of all executable sources:
{\footnotesize
\begin{verbatim}
git clone https://github.com/joewandy/MS2LDA.git
cd MS2LDA
git checkout eba1ac9a9f4b89c1f35b4f66a57e3f8590f2abb8
...
because multicore partition training is nondeterministic. The replacement
therefore reuses that model only through its strict hash-audited Tomotopy-only
import; a completely fresh reconstruction preserves the
...
physical peak contexts in its structured prior. A subsequent audit invalidated
the nearest-state correction because peaks discarded by DreaMS could borrow a
nearby retained state. Both results have been removed. The replacement
full-data seed-42 protocol preserves the stricter scaffold split, MAG exclusion,
full corpus and $K=1000$, but requires exact retained peak identity and rebuilds
every Hybrid-dependent artifact. No chemical or speed/coherence conclusion is
reported until that run and its independent audits complete. Any eventual
single-seed result will remain post-hoc indicative evidence, not a universal
ranking, proof of individual fragment assignments, substitute for independent
manual validation, or production-backend decision.
