"""Command-line interface for the fully neural MS2LDA study."""
...
def _parser() -> argparse.ArgumentParser:
    ...
    run = commands.add_parser("run", help="run or resume the bounded study")
    ...
    progress = commands.add_parser("status", help="show a progress snapshot")
    progress.add_argument("--run", required=True, type=Path)
    verify = commands.add_parser("verify", help="verify run provenance")
    ...
    smoke = commands.add_parser("smoke", help="run a tiny mechanics check")
