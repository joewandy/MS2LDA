...
from .model import AssignmentOutput, NeuralAssignmentMS2LDA
...
def router_block_loss(  # noqa: PLR0913
    ...
    *,
    ...
    """
    left = model.route(
        left_batch, temperature=temperature, top_k=top_k, straight_through=True
    )
    right = model.route(
        right_batch, temperature=temperature, top_k=top_k, straight_through=True
    )
    completion = 0.5 * (
...
def _local_decoder_loss(beta: torch.Tensor, output: AssignmentOutput) -> torch.Tensor:
