...
from .baseline_repeats import (
    baseline_fit_identity,
    frozen_training_input_hashes,
    require_cached_fit,
...
from .test_release import verify_released_model
from .utils import (
    atomic_save_numpy,
    read_json,
    sha256_file,
...
def train_tomotopy(  # noqa: PLR0915
    ...
    *,
    ...
    tp = _tomotopy()
    seed = resolve_training_seed(protocol, training_seed)
    identity = baseline_fit_identity(
        directory,
        seed,
        {
            "topics": int(protocol["model"]["num_topics"]),
            "tomotopy": dict(protocol["tomotopy"]),
            "version": tp.__version__,
            "alpha_optimization_interval": ALPHA_OPTIMIZATION_INTERVAL,
        },
    )
    final_model = output / "model.bin"
    if complete_path.is_file():
        cached = read_json(complete_path)
        require_cached_fit(cached, identity)
        if not final_model.is_file() or cached.get("model_sha256") != sha256_file(
            final_model
        ):
            raise RuntimeError("cached Tomotopy checkpoint is missing or changed")
        return cached
    if final_model.exists():
        raise RuntimeError(
            "incomplete Tomotopy fit exists; use a fresh attempt directory"
        )
    ...
    # Publish the binary atomically, then publish its completion marker below.
    # An interrupted save must never leave a complete-looking partial model.
    temporary = output / f".model.bin.{os.getpid()}.tmp"
    try:
...
def evaluate_tomotopy(  # noqa: PLR0915
    ...
    *,
    ...
    training_path = directory / "tomotopy/complete.json"
    if not training_path.is_file() or not model_path.is_file():
        raise RuntimeError(
            "Tomotopy evaluation requires a complete training checkpoint"
        )
    training = read_json(training_path)
    if not isinstance(training.get("fit_identity"), dict):
        raise RuntimeError("Tomotopy training cache lacks a verifiable fit identity")
    recipe = {
        **training["fit_identity"]["recipe"],
        "topics": int(protocol["model"]["num_topics"]),
        "tomotopy": dict(protocol["tomotopy"]),
    }
    if split == "test":
        # Releasing test data authorizes evaluation of one frozen checkpoint,
        # never another fit. The canonical gate rehashes every released input.
        verify_released_model(directory, method="tomotopy", model_path=model_path)
        expected_fit = {
            "training_seed": training["training_seed"],
            "recipe": recipe,
            "input_sha256": frozen_training_input_hashes(directory),
        }
    else:
        expected_fit = baseline_fit_identity(
            directory, training["training_seed"], recipe
        )
    require_cached_fit(training, expected_fit)
    if training.get("model_sha256") != sha256_file(model_path):
        raise RuntimeError("Tomotopy training checkpoint changed after completion")
    artifacts = {
        "training_complete.json": training_path,
        "model.bin": model_path,
        f"{split}_full_theta.npy": output / f"{split}_full_theta.npy",
    }
    if split == "validation":
        artifacts["beta.npy"] = output / "beta.npy"
    if complete_path.is_file():
        if not all(path.is_file() for path in artifacts.values()):
            raise RuntimeError(
                "cached Tomotopy evaluation is missing required artifacts"
            )
        cached = read_json(complete_path)
        hashes = {name: sha256_file(path) for name, path in artifacts.items()}
        if (
            cached.get("fit_identity") != expected_fit
            or cached.get("artifact_sha256") != hashes
            or cached.get("split") != split
        ):
            raise RuntimeError(
                "cached Tomotopy evaluation identity or artifacts differ"
            )
        return cached
