...
from benchmarks.neural_ms2lda import evidence_bundle
from benchmarks.neural_ms2lda import reproduction_comparison as comparisons
from benchmarks.neural_ms2lda import reproduction_summaries as summaries
...
def test_fresh_chemistry_requires_explicit_mag_exception_counts() -> None:
    ...
    with pytest.raises(RuntimeError, match="lacks explicit MAG exception"):
        comparisons.chemistry_summary(result)


def test_packager_rejects_non_cuda_neural_evidence() -> None:
    summaries.require_neural_device("cuda", label="test fixture")
    with pytest.raises(RuntimeError, match="was not executed on cuda"):
        summaries.require_neural_device("mps", label="test fixture")
...
def test_packaged_json_paths_are_made_host_independent(tmp_path: Path) -> None:
    ...
    evidence_bundle.rewrite_json_as_portable(package, replacements)
    evidence_bundle.assert_no_machine_paths(package)
...
def test_chemical_integrity_gate_includes_every_contextual_seed() -> None:
    ...
    result = summaries.chemical_integrity_checks(comparison, stability)
...
def test_high_k_claim_uses_truth_matched_recovery_not_winner_count() -> None:
    ...
    result = summaries.claim_checks(*evidence)
