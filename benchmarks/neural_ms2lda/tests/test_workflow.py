...
from benchmarks.neural_ms2lda import chemical, validation_data
...
def test_etm_control_loader_opens_validation_data_only(
    ...
    monkeypatch.setattr(validation_data, "load_csr", recording_load)
    loaded = validation_data.load_validation_inputs(view)
    assert loaded.train.shape[1] == len(loaded.vocabulary)
    assert opened == [
        "train.npz",
