...
class CanonicalETM(nn.Module):
    ...
    Paper-to-code state: rho[w] is a fixed word vector (V x E),
    alphas.weight[k] is a learned topic vector (K x E), and encoder/mu/logvar
    represent the two-layer MLP and Gaussian heads in eq:base-posterior.
    These layers are inherited ETM components, not the spectral enhancements.
    """
    ...
    def topic_word_distribution(self) -> torch.Tensor:
        """Return K x V beta from ``eq:base-beta`` in the current manuscript.

        alphas(rho) produces V x K dot products rho_w^T alpha_k.
        Softmax along dim=0 normalizes over WORDS, separately for each topic;
        transposition then makes each returned topic row sum to one.
        """
    ...
    def posterior(
        ...
        """Return mu_phi(x), ell_phi(x), and KL[q(z|x) || N(0,I)] per spectrum.

        Both heads are B x K. logvar stores log VARIANCE (ell), not log
        standard deviation: sampling uses exp(ell/2), while KL uses exp(ell).
        """
