...
def _topic_scores(
    *,
    ...
    """Apply current ``eq:sos`` with one dominant-topic association per spectrum.

    For each topic k, C_k contains unique associated connectivity groups with
    valid molecular fingerprints. A_k is the MAG consensus fingerprint, formed
    only for an optimized motif with valid retrieved fingerprint data. An
    all-zero A_k is available evidence and receives overlap zero; unavailable
    consensus (None) is NOT an empty set and makes the motif unevaluable.
    Evaluable topics therefore require optimization, available consensus and
    nonempty C_k. Mean SOS is unweighted across those evaluable topics.
    """
    associated = _associated_record_indices(theta)
    if len(records) != theta.shape[0]:
        raise ValueError("records must match every theta row")
    topic_ids = [row["topic_id"] for row in annotations]
    if len(topic_ids) != theta.shape[1] or set(topic_ids) != set(range(theta.shape[1])):
        raise ValueError("annotations must identify every topic exactly once")
    if not np.isfinite(fingerprint_threshold) or not 0 <= fingerprint_threshold <= 1:
        raise ValueError("fingerprint threshold must be finite and in [0, 1]")
...
def run_chemical_scoring(
    ...
    *,
    ...
    if method == "minimal_etm" and split != "validation":
        raise ValueError("minimal ETM is a validation-only development experiment")
