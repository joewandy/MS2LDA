...
def _completion_views(
    ...
    *,
    ...
    """Split paired peaks without changing their already-generated integer counts.

    SyntheticPeak.count was rounded using the full synthetic spectrum's maximum
    intensity in _draw_document. Unlike real-data renormalize_peak_groups, this
    historical protocol does NOT re-max-normalize or re-discretize the observed
    half. The topic-model encoder later normalizes each observed count row.
    Thus observed + withheld counts exactly reconstruct the generated document,
    while rounding can retain a dependence on its original intensity maximum.
    Keep this distinction explicit when comparing real and synthetic completion.
    """
...
def matched_truth_metrics(
    ...
    """Align all R planted motifs to K >= R fitted topics, then score recovery.

    beta arrays have shapes K x V (fitted) and R x V (truth); theta arrays
    share D spectrum rows and have K or R columns, respectively. Rows must
    contain finite nonnegative values and positive mass. Unit row sums are not
    required here: cosine is invariant to positive row scaling.

    The Hungarian assignment maximizes the sum of word-distribution cosines.
    Word recovery averages ALL R matched cosines, and the recovery count uses
    the inclusive threshold 0.50. Mixture recovery keeps only the R matched
    fitted columns, aligned into R truth columns. Unmatched mass is not directly
    penalized; a numerically zero aligned vector receives cosine zero.

    Reject K < R rather than silently averaging only a subset of the planted
    motifs. The guards below do not renormalize or alter valid saved inputs.
    """
    learned_beta, learned_theta, true_beta, true_theta = (
        np.asarray(values)
        for values in (learned_beta, learned_theta, true_beta, true_theta)
    )
    for name, values in (
    ...
    if learned_beta.shape[1] != true_beta.shape[1]:
        raise ValueError("fitted and true beta must share the same vocabulary")
    if learned_beta.shape[0] < true_beta.shape[0]:
        raise ValueError("fitted topic count K must be at least planted count R")
    if learned_theta.shape[1] != learned_beta.shape[0]:
        raise ValueError("learned theta columns must match fitted topics")
    if true_theta.shape[1] != true_beta.shape[0]:
        raise ValueError("true theta columns must match planted topics")
    if learned_theta.shape[0] != true_theta.shape[0]:
        raise ValueError("fitted and true theta must share the same spectrum rows")
    epsilon = 1e-12
