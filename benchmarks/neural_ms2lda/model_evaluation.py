...
def completion_metrics(
    ...
    """Score token-weighted completion using mixture probabilities theta @ beta.

    Current equation ``eq:completion`` uses log(max(p, 1e-12)), matching the
    training reconstruction's numerical floor. The denominator counts only
    in-vocabulary withheld pseudo-tokens; this is not a document-mean score.
    A row is eligible whenever its withheld count mass is positive, even if
    its observed half had no in-vocabulary counts and required an encoder
    fallback. Only zero-withheld-mass rows are omitted from this average.
    """
...
def theta_support_diagnostics(theta: np.ndarray) -> dict[str, object]:
    ...
    In current ``eq:effective``, K_eff = exp(-sum_k theta_k log(theta_k)).
    As in completion, positive probabilities inside logs are floored at 1e-12;
    zero weights contribute exactly zero. The reported median is over spectra
    WITHIN one fit, not a median over training repetitions. Exact support counts
    every positive entry, including those smaller than the logarithm floor.
    """
