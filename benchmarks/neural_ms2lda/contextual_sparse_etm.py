...
This module preserves the original frozen leave-one-out model and the shared
equation functions. The current report selects the whole-spectrum-context
subclass in ``contextual_reductions.py``; the complete historical description
is archived in ``research/minimal_neural_etm/review_20260907/``. Shared helper
docstrings name equations in ``docs/research/contextual_sparse_etm_supplement.tex``;
the companion ``contextual_sparse_etm_report.tex`` provides the main explanation.
Leave-one-out helpers explicitly refer to the archived equations instead.
Shared notation:
...
def unit_normalize_rows(vectors: torch.Tensor) -> torch.Tensor:
    ...
    geometry used by current ``eq:document-context`` and its cosine scores easy
    to inspect. It is also used by the archived leave-one-out context. A zero
    vector stays zero; the epsilon floor is not a learned geometric parameter.
    """
...
def leave_one_out_context(
    ...
    """Compute observed-word context from archived equation ``eq:loo-context``.
...
def contextual_top2_evidence(
    ...
    ``eq:document-evidence`` in the ARCHIVED leave-one-out report. The current
    paper uses whole-spectrum context in contextual_reductions.pooled_evidence.
    ...
    # h_dw from the archived equation (eq:contextual-word).
    contextual_words = unit_normalize_rows(
        rho_hat[word_indices] + context_scale * rho_bar,
    )

    # a_dwk from archived equation (eq:context-score). Temperature is fixed at one
    ...
    # q_dwk from the archived equation (eq:top2-assignment).
...
def centered_log_evidence_offset(evidence: torch.Tensor) -> torch.Tensor:
    ...
    # A float32 reduction can leave a uniform row at roughly 1e-8, not zero.
    # Remove only that forward-rounding residual. Detaching the correction is
    # essential: the centered-log equation still has a NONZERO derivative at
    # uniform evidence, (K/2) * (I - 11^T/K). Returning a literal zero branch
    # would incorrectly stop gradients to evidence at symmetric routing states.
    uniform_rows = torch.all(evidence == evidence[:, :1], dim=1, keepdim=True)
    correction = torch.where(uniform_rows, offset.detach(), torch.zeros_like(offset))
...
def diagonal_gaussian_kl(
    ...
    Inputs have shape B x K; output has shape B. In the enhanced model,
    posterior_mean is the FULL shifted mean m, including contextual evidence.
    The algebra is 0.5 * sum_k(m_k^2 + exp(ell_k) - 1 - ell_k).
    The prior and posterior are distributions over z, not over entmax(z).
    """
...
def reparameterized_gaussian(
    ...
    *,
    ...
    """Sample the Gaussian in current ``eq:base-posterior`` / ``eq:elbo``.

    Inputs m and ell have shape B x K; ell is log variance. Training uses
    z = m + exp(ell / 2) * epsilon with epsilon ~ N(0,I), one independent draw
    per spectrum. Deterministic inference sets z = m; applying entmax to m is
    a plug-in summary, not an expectation over Gaussian samples.
    """
    if not sample:
...
def entmax15_document_mixture(latent_logits: torch.Tensor) -> torch.Tensor:
    ...
    Current equation ``eq:entmax`` gives theta_k = [z_k/2 - tau(z)]_+^2,
    where tau makes the row sum one. Inputs and outputs both have shape B x K.
    No support cutoff or additional sparsity penalty is applied.
    """
...
class ContextualSparseETM(nn.Module):
    ...
    def posterior(
        ...
        In the ARCHIVED leave-one-out model this is the executable sequence of
