...
from .utils import read_json_object as read_json_object
from .utils import sha256_file as sha256_file
...
def sample_runtime_memory(previous: MemoryState | None = None) -> MemoryState:
    ...
    # Linux reports KiB; macOS reports bytes for the same resource field.
    rss_unit_bytes = 1 if sys.platform == "darwin" else 1024
    process_bytes = (
        int(resource.getrusage(resource.RUSAGE_SELF).ru_maxrss) * rss_unit_bytes
    )
...
def runtime_memory_metrics(
    ...
    return {
        "measurement": (
            "sampled per epoch; process is platform-normalized ru_maxrss and system is "
