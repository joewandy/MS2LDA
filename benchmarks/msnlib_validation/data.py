    ...
    if rm_top:
        removed = {
            ...
            for word, _ in sorted(
                ((word, corpus_frequency[word]) for word in retained),
                key=lambda item: (-item[1], item[0]),
            )[:rm_top]
        }
        retained = [word for word in retained if word not in removed]
    vocabulary = tuple(sorted(retained))
    if not vocabulary:
...
def to_matchms_spectrum(record: SpectrumRecord):
    """Convert one already-cleaned record into a DreaMS-compatible Spectrum."""
    try:
        from matchms import Spectrum
    except ImportError as exc:  # pragma: no cover - environment validation
        raise ImportError("matchms is required for DreaMS feature extraction") from exc
    return Spectrum(
        mz=np.asarray([group.mz for group in record.peak_groups], dtype=np.float64),
        intensities=np.asarray(
            [group.intensity for group in record.peak_groups], dtype=np.float64
        ),
        metadata={
