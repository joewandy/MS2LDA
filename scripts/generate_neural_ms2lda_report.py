...
def generated_text(evidence: dict[str, Any], protocol: dict[str, Any]) -> None:
    ...
    code_rows = (
        ...
        ("Nonlinear leave-one-out context", r"\texttt{model.py: \_route\_embeddings}"),
    ...
    fit_ratio = tomotopy["fitting_seconds"] / neural["fitting_seconds"]
    neural_coverage = 100 * neural_validation["annotation_coverage"]
    tomotopy_coverage = 100 * tomotopy_validation["annotation_coverage"]

    def useful_share(metrics: dict[str, object]) -> float:
        return 100 * (
            metrics["useful_high_confidence_motifs"]
            / metrics["high_confidence_evaluable_motifs"]
        )
    ...
    values = {
        ...
        "NeuralValidationMedianSOS": (f"{neural['validation']['median_sos']:.4f}"),
        "TomotopyValidationMedianSOS": (f"{tomotopy['validation']['median_sos']:.4f}"),
        ...
        "NeuralTestMedianSOS": f"{neural['test']['median_sos']:.4f}",
        "TomotopyTestMedianSOS": f"{tomotopy['test']['median_sos']:.4f}",
        "NeuralCoverage": f"{neural_coverage:.1f}\\%",
        "TomotopyCoverage": f"{tomotopy_coverage:.1f}\\%",
        "CoveragePointDelta": f"{neural_coverage - tomotopy_coverage:.1f}",
        "ValidationOptimizedDelta": (
            neural["validation"]["optimized_motifs"]
            - tomotopy["validation"]["optimized_motifs"]
        ),
        "ValidationEvaluableDelta": (
            neural["validation"]["high_confidence_evaluable_motifs"]
            - tomotopy["validation"]["high_confidence_evaluable_motifs"]
        ),
        "ValidationUsefulDelta": (
            neural["validation"]["useful_high_confidence_motifs"]
            - tomotopy["validation"]["useful_high_confidence_motifs"]
        ),
        "TestEvaluableDelta": (
            neural["test"]["high_confidence_evaluable_motifs"]
            - tomotopy["test"]["high_confidence_evaluable_motifs"]
        ),
        "TestUsefulDelta": (
            neural["test"]["useful_high_confidence_motifs"]
            - tomotopy["test"]["useful_high_confidence_motifs"]
        ),
        "NeuralValidationUsefulShare": (f"{useful_share(neural['validation']):.1f}\\%"),
        "TomotopyValidationUsefulShare": (
            f"{useful_share(tomotopy['validation']):.1f}\\%"
        ),
        "NeuralTestUsefulShare": (f"{useful_share(neural['test']):.1f}\\%"),
        "TomotopyTestUsefulShare": (f"{useful_share(tomotopy['test']):.1f}\\%"),
        "NeuralFitMinutes": f"{neural['fitting_seconds'] / 60:.1f}",
        "TomotopyFitMinutes": f"{tomotopy['fitting_seconds'] / 60:.1f}",
        "CoreFitRatio": f"{fit_ratio:.2f}",
        ...
        "ValidationNLLDelta": (
            f"{nll['tomotopy']['validation'] - nll['neural']['validation']:.4f}"
        ),
        "NeuralTestNLL": f"{nll['neural']['test']:.4f}",
        "TomotopyTestNLL": f"{nll['tomotopy']['test']:.4f}",
        "TestNLLDelta": (f"{nll['tomotopy']['test'] - nll['neural']['test']:.4f}"),
