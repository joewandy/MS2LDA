...
class TestCreateSpectrum:
    ...
    def test_create_spectrum_custom_tags(self):
        ...
        spectrum = utils.create_spectrum(
            features, k=0, frag_tag="fragment@", loss_tag="neutral_loss@"
        )
    ...
    def test_create_spectrum_metadata(self):
        ...
        spectrum = utils.create_spectrum(
            features, k=5, charge=2, motifset="test_motifs"
        )
    ...
    def test_create_spectrum_significant_digits(self):
        ...
        spectrum = utils.create_spectrum(features, k=0, significant_digits=2)

        assert spectrum.peaks.mz.tolist() == pytest.approx([123.46])
        assert spectrum.losses.mz.tolist() == pytest.approx([45.68])
...
class TestMatchFragsAndLosses:
    ...
    def test_match_frags_and_losses_multiple_spectra(self):
        ...
        # Use Mass2Motif because regular Spectrum losses are derived differently.
        from MS2LDA.Mass2Motif import Mass2Motif
...
class TestRetrieveSpec4Doc:
    ...
    def test_retrieve_spec4doc_missing_mapping(self):
        ...
        doc2spec_map = {
            "wrong_key": Spectrum(mz=np.array([100.0]), intensities=np.array([1.0]))
        }
