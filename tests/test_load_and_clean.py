...
class TestCleanSpectra:
    ...
    def test_clean_spectra_basic(self, sample_spectra_list):
        ...
        cleaned = clean_spectra(
            iter(sample_spectra_list[:-1])
        )  # Exclude empty spectrum
    ...
    def test_clean_spectra_combined_filters(self):
        ...
        for i in range(10):
            ...
            intensities = rng.random(n_peaks) * (i + 1) * 0.1
            spectra.append(
                Spectrum(
                    mz=mz,
    ...
    @pytest.mark.parametrize(
        "param_name,param_value,expected_count",
        [
...
class TestIntegration:
    ...
    def test_mixed_file_loading(self, temp_dir, sample_mgf_content, sample_msp_content):
        ...
        assert [s.get("id") for s in cleaned] == [
            "spec_0",
            "spec_1",
            "spec_2",
            "spec_3",
